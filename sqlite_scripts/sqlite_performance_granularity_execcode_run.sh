...
rm $localDir/TypeChef-SQLiteIfdeftoif/sqlite_performance_granularity_execcode.sh
cp sqlite_performance_granularity_execcode.sh $localDir/TypeChef-SQLiteIfdeftoif/

cd $localDir
cd TypeChef-SQLiteIfdeftoif
# Use java 8
export PATH=/usr/lib/jvm/jdk-8-oracle-x64/bin/:$PATH
./sqlite_performance_granularity_execcode.sh ${SLURM_ARRAY_TASK_ID}
